...
[2. Extracting annotation](#extracting-annotation)
...
Visualising all associations at once can be done with a Manhattan plot, where the x-axis represents chromosome`CHR` and the chromosomal position `POS`, and the y-axis the `-log10(P)`-value. Let's use the R-package [`ggman` by Veera Manikandan](https://github.com/drveera/ggman) for that. <!-- (creating an immediate `r fa_smile(colour = "gray")` or `r fa_frown(colour = "gray")` effect)-->
...
`r fa_exclamation()` `biomaRt` should not to be confused with [`biomartr`](https://github.com/ropensci/biomartr). `biomartr` is an rOpenSci package by [Hajk-Georg Drost](https://twitter.com/HajkDrost). I needed this tweet `r fa_arrow_down(colour = "black")` to realise that these were two separate, yet related R-packages. 
...
## extract gene
## ----------
out.bm.snp2gene <- getBM(attributes = c('refsnp_id', 'allele', 'chrom_start', 'chr_name', 'ensembl_gene_stable_id'), 
...
Let's try to make that pretty. We can group the genes by `gene_biotype` and colour them accordingly. The protein coding genes are in the top row.
...
- ... to polish that plot even more? For example, the colour code is not ideal...
