...
listAttributes(variation) %>% slice(str_which(description, "symbol")) 
listFilters(snpmart)
...
```{r, get-snp-info-pos-chr, include = TRUE, cache=TRUE}
...
```{r, get-gene-info, include = TRUE,cache =TRUE}
...
```{r, get-all-snps-in-range, include=TRUE, cache=TRUE}
