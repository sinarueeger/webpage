...
plot(dat.bmi.sketch$POS, -log10(dat.bmi.sketch$Pvalue), xlab = "Position (bp)", ylab = "-log10(P-value)", ylim = c(-10, 40), yaxt="n")
...
Unless you sit on lots of genetic data, the list of SNPs come from a summarised form. Minimally, these summary statistic datasets contain the SNP identifier (`SNPNAME`, for example `rs149969323`), the effect size (`beta`) and the standard error (`se`). Sometimes the position (`POS`) and chromosome (`CHR`) is provided instead of the SNP identifier, and sometimes both are available. Then there is usually other information coming from the study data, for example the allele frequency in the study `r icon::ii_arrow_right_a()` this is on the LHS in table below. However, hardly ever do the datasets contain annotation, such as the [gene](https://www.cancer.gov/publications/dictionaries/cancer-terms/def/gene) where the SNP resides, the allele frequency in a certain population or the diseases it is associated with `r icon::ii_arrow_right_a()` this is illustrated in the <font color="#31a354">green</font> colored part in table below.
...
library(kableExtra)
kable(out.bm.table %>% as_tibble %>% dplyr::select(CHR, POS, SNPNAME, REF, ALT, beta, se, external_gene_name, phenotype_name, minor_allele_freq) %>% dplyr::rename(Gene = external_gene_name, Phenotype = phenotype_name, `Global minor allele freq.` = minor_allele_freq)) %>%
...
In the illustration below^[Thanks to [Maëlle Salmon](https://twitter.com/ma_salmon) you get a real plot here instead of a blurry hand drawing. She gave me lots of feedback on my first version - much appreciated!], we want to know the gene starting and end positions around one particular gene region to create an informative locuszoom plot. 
...
Annotation of a SNP can be its gene, the function of the gene, which which phenotypes the gene is associated. 
...
Thanks to [Marianna Foos](https://twitter.com/MariannaFoos) for pointing out another great R-package dealing with SNP annotation called [`rsnps`](https://cran.r-project.org/web/packages/rsnps/vignettes/rsnps_vignette.html)!
...
Next we specify which database to use. You can use the functions `listMart()`, `listEnsembl()` and `listDatasets()` to select from the right biomart and dataset. We will need to extract SNPs, hence the `biomart = "snp"`.
