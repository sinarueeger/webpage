...
In the illustration below, we want to know the gene starting and end positions around the FTO gene region to make some sort of locus zoom plot. 

<figure>
<img src="/post/annotation/locuszoom-drawing.jpg" alt="Drawing of a table." align="middle" style="width: 400px;"/>
<figcaption><small>In this case, we want to visualise the GWAS summary statistics of a genomic region (with each point representing a SNP), with the corresponding genes at the bottom.</small></figcaption>
</figure>
...
```{r, load-data, echo=TRUE, results="hide", cache=TRUE, message = FALSE}
...
The reference allele (`REF`), the alternative allele (`ALT`) and the SNP identifier (`SNP`) are characters (there are four unique values for `ALT` and `REF`: A, C, G, T). The two columns with the minor allele frequencies measured in [GIANT](https://portals.broadinstitute.org/collaboration/giant/index.php/Main_Page) and [ExAC](http://exac.broadinstitute.org/) datasets (`GMAF`, `ExAC_MAF`) are characters too, because the allele is concatenated. The columns chromosome (`CHR`) and the chromosomal position (`POS`) are integers. Then there is the actual association of that SNP with BMI (`beta`, `se`, `P`).
...
This dataset has `r nrow(dat.bmi)` rows and `r ncol(dat.bmi)` columns. What if we want to visualise this all at once? In particularly, we are interested if there are ANY associations between the genetic markers and BMI. The mini `P` distribution in the skimr output looks almost uniform. 


### Visualising associations

Visualising all associations at once can be done with a Manhatten plot (having an immediate `r fa_smile(colour = "gray")` or `r fa_frown(colour = "gray")` effect), where the x-axis represents chromosome`CHR` and the chromosomal position `POS`, and the y-axis the `-log10(P)`-value. Let's use the R-package [`ggman`](https://github.com/drveera/ggman) for that.
...
```{r, define-bmi-min, warning =FALSE}
...
SNP identifier `r dat.bmi.min$SNP` that has the lowest P-value (P = `r dat.bmi.min$P`). 

```{r, range, include = TRUE}
range <- 500e3
```

Now we can visualise the summary statistics of that genomic region ($\pm$ `r range`). 

```{r, prep-min-data, include = TRUE}
...
p1 <- qplot(POS, -log10(P), data = dat.bmi.fto.region) + labs(title = "Locuszoomplot for BMI GWAS",
       subtitle = paste("Chromosome", fto.chr, "from", format((fto.pos - range), big.mark = "'") , "to", format((fto.pos + range), big.mark = "'") ), caption = paste("Data source:", url))
print(p1)
...
Our BMI example is from human data (aka homo sapiens), but the R-package can actually expand on any other commonly studied species. In fact, compared to what `biomaRt` is capable of, I am only going to scratch the surface a tiny bit. 

A bit of **background**: Previously, I used other R-packages from CRAN and bioconductor to annotate SNPs, but they all became obsolete eventually. And so I was incredibly happy I stumbled over this package a few weeks ago^[Meaning = I have not used it extensively = I am not an expert.]. 

`r fa_exclamation()` `biomaRt` should not to be confused with `biomartr`. `biomartr` is is an rOpenSci package by [Hajk-Georg Drost](https://twitter.com/HajkDrost). I needed this tweet `r fa_arrow_down(colour = "black")` to realise that these were two separate, yet related R-packages. 
...
So far, I did not fully grasp the benefits of `biomartr` compared to `biomaRt` for annotation of homo sapiens data. But I will definitely [look more into it](https://ropensci.github.io/biomartr/articles/Functional_Annotation.html).
...
Before getting the gene names, I want to check if my chromosome and position in the dataset are actually matching with the info in the database. This is a handy thing, because oftentimes, only SNPs are reported. Or SNPs are reported, but with positions on a different build. 
...
So now we know that `r dat.bmi.min$SNP` is part of `r out.bm.ensembl$external_gene_name`. But where does `r out.bm.ensembl$external_gene_name` start and end? And what are the genes nearby? For this purpose, we want to visualise the summary statistics of the full genomic region ($\pm 250e3$). 

```{r, get-all-genes-in-range, include=TRUE, cache=TRUE}

listAttributes(ensembl) %>% slice(str_which(description, "Gene type")) 
...
```{r, plot-genes-pretty, include = TRUE, fig.height = 3}

## define plot range
plot.range <- c(min(fto.pos - range, out.bm.genes.region$start_position), max(fto.pos + range, out.bm.genes.region$end_position))
...
There are some genes starting with *AC-* and *LINCO* and *RINU*. It is tempting to remove them, since they are so small. So let's [check](https://www.genenames.org/about/guidelines) what they are. 

What would be great, is an interactive plot. Like, hovering over the points and the corresponding gene lights up. But do I have time for this? No :-)
...
An easy solution is [LocusZoom](http://locuszoom.org/), a tool that takes summary statistics as input and outputs a pretty graph for a desired genomic region, including gene annotation, LD information etc. 
...
The RMarkdowns file is [here](https://github.com/sinarueeger/webpage/blob/master/blogdown/content/post/2018-07-04-plotting-local-genomic-association-results-in-r.Rmd).