...
The 3+ days at [useR!2019 in Toulouse](http://www.user2019.fr) were packed with great talks^[I cannot wait to see the recordings to catch up with the parallel sessions that I missed!]. 

Here is an attempt to list some free appetizers ([amuse-bouches](https://en.wikipedia.org/wiki/Amuse-bouche)) from the conference. Hopefully convincing enough to start using a new package or change a workflow. Not everything was brand-new, but it was helpful to have someone talking you through their inspiration and examples. 
...
I never warmed up to the bang-bangs and enquo's. Hence the new and more straight forward "curly-curly"'s for functional programming `{{ arg }}` in the tidyverse feel like a game-changer. 
...
You can use the "curly-curly" brackets if you want to turn this small bit of code into a function `group_mean()` with a `data`, `by` and `var` argument^[See [Slide 49+](https://speakerdeck.com/lionelhenry/reusing-tidyverse-code?slide=49).] (want to pass the variables on in an unquoted way):
...
`usethis` is all about avoiding to copy+pasting. For example, there is a function to edit the `.Rprofile` called `usethis::edit_r_profile()`. Whenever there is a slightly complicated task ahead (_restarting R_), the `usethis` package will talk you through the whole process. 
...
What a history reshaping data in R already has! From `reshape` to `melt`+`cast`, over to `gather`+`spread` and now `pivot_long` and `pivot_wide`. Reshaping data stays a mind-bending task, but hopefully, these `pivot_*` functions will make life easier.
...
Importing large datasets into R can be a painful task. Especially if you only need a subset of the columns. And apparently, our thoughts drift off after 10 sec^[I can totally confirm that.] starring at the screen where it is _still_ loading the dataset.
...
The `rstatsmeme` package is a little gem discovered thanks to [Frie](https://twitter.com/ameisen_strasse):
